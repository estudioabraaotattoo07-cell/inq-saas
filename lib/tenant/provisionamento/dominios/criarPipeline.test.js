...
test("garante as 14 etapas padrão com sucesso", async () => {
  const sb = criarSbFalso();
  const resultado = await criarPipeline(sb, DADOS_VALIDOS);
  assert.equal(resultado.status, "garantido");
  assert.equal(resultado.detalhe.etapasProcessadas, 14);
  assert.equal(sb._chamadas.length, 1);
  assert.equal(sb._chamadas[0].payload.length, 14);
});

test("lead_morno e aura_agend nao estao mais no payload provisionado (Bloco de Unificacao, 2026-08-14)", async () => {
  const sb = criarSbFalso();
  await criarPipeline(sb, DADOS_VALIDOS);
  const slugs = sb._chamadas[0].payload.map((p) => p.slug);
  assert.equal(slugs.includes("lead_morno"), false);
  assert.equal(slugs.includes("aura_agend"), false);
});

test("lead vem com o label 'Clientes interessados'", async () => {
  const sb = criarSbFalso();
  await criarPipeline(sb, DADOS_VALIDOS);
  const lead = sb._chamadas[0].payload.find((p) => p.slug === "lead");
  assert.equal(lead.label, "Clientes interessados");
});

test("todas as 14 linhas recebem o user_id correto", async () => {
  const sb = criarSbFalso();
  await criarPipeline(sb, DADOS_VALIDOS);
...
test("ordem enviada é 1 a 14, na sequência do array canônico, sem lacunas", async () => {
  const sb = criarSbFalso();
  await criarPipeline(sb, DADOS_VALIDOS);
  const payload = sb._chamadas[0].payload;
  assert.deepEqual(payload.map((p) => p.ordem), Array.from({ length: 14 }, (_, i) => i + 1));
});
...
test("as outras 13 etapas têm fixo=true", async () => {
  const sb = criarSbFalso();
  await criarPipeline(sb, DADOS_VALIDOS);
  const payload = sb._chamadas[0].payload;
