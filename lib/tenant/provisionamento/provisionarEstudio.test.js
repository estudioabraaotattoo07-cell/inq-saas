...
import { criarConfiguracoes } from "./dominios/criarConfiguracoes.js";
...
test("integração: sem terceiro parâmetro, usa o registro real (DOMINIOS_ATIVOS) e executa os cinco domínios congelados de ponta a ponta", async () => {
  ...
  assert.equal(relatorio.etapas.length, 5);
  ...
  assert.equal(relatorio.etapas[3].etapa, "pipeline");
  assert.equal(relatorio.etapas[3].status, "garantido");
  assert.equal(relatorio.etapas[4].etapa, "configuracoes");
  assert.equal(relatorio.etapas[4].status, "garantido");
  assert.equal(relatorio.sucesso, true);
  ...
  assert.equal(tabelaPipelineEtapas.length, 14);
  assert.equal(tabelaConfiguracoes.size, 1);
  assert.deepEqual(Object.keys(tabelaConfiguracoes.get(DADOS.authUserId)), ["id", "user_id"]);

  // Confirma que criarIdentidade/criarLicenciamento/criarFinanceiro/criarPipeline/
  // criarConfiguracoes continuam com a mesma assinatura que o registro espera
  ...
  assert.equal(typeof criarPipeline, "function");
  assert.equal(typeof criarConfiguracoes, "function");
});
